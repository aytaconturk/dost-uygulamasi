...
export default function StoryList({ stories }) {
    ...
    return (<div className="text-white relative">
            ...
            {showPrev && (<button onClick={() => scroll('left')} className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-purple-500 hover:bg-purple-700 text-white w-10 h-10 rounded-full flex items-center justify-center shadow-lg z-10">
                    <ChevronLeft size={24}/>
                </button>)}
            {showNext && (<button onClick={() => scroll('right')} className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-purple-500 hover:bg-purple-700 text-white w-10 h-10 rounded-full flex items-center justify-center shadow-lg z-10">
                    <ChevronRight size={24}/>
                </button>)}

            <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4 px-12 scroll-smooth [&::-webkit-scrollbar]:hidden scrollbar-hide">
                {stories.map((story) => (story.locked ? (<div key={story.id} className="relative w-full sm:w-1/2 md:w-1/3 lg:w-1/4 min-w-[240px] max-w-[280px] bg-white text-black rounded-xl shadow-md opacity-80 blur-[1px]">
