...
export default function ReadingScreen({ stories }) {
    ...
    useEffect(() => {
        return () => {
            console.log('🧹 ReadingScreen cleanup: Stopping all audio');
    ...
    return (<div className="min-h-screen bg-[#f9f9fb] flex flex-col relative top-[-24px]">
            <audio ref={audioRef} preload="auto"/>

            {/* Progress Bar */}
            <div className="w-full h-2 flex bg-gray-200">
                {steps.map((_, stepIndex) => (<div key={stepIndex} className={`flex-1 transition-all ${stepIndex < currentStep ? 'bg-green-500' :
            ...
            {completedSteps.some(completed => completed) && (<div className="bg-green-50 border-b border-green-200 py-3 px-6">
                    <div className="max-w-4xl mx-auto">
                        <h3 className="text-sm font-semibold text-green-800 mb-2">Tamamlanan Adımlar:</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {steps.map((step, index) => (<div key={index} className="flex items-center gap-2">
                                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${completedSteps[index]
                    ...
                    : index === currentStep
                        ...
                        : 'border-gray-300'}`}>
                                    ...
                                    <span className={`text-sm ${completedSteps[index]
            ...
            {/* Başlık ekranı */}
            {!stepStarted && (<div className="flex-1 flex flex-col items-center justify-center">
                    <h2 className="text-2xl font-semibold text-purple-800 mb-4">
                        {steps[currentStep].title}
                    </h2>
                    <button onClick={() => setStepStarted(true)} className="bg-purple-600 text-white px-6 py-3 rounded-full shadow hover:bg-purple-700 transition">
                        Başla
                    </button>
                </div>)}

            {/* İçerik ekranı */}
            {stepStarted && (<motion.div initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="flex-1 flex flex-col md:flex-row items-center justify-center gap-8 px-4 md:px-16 relative mt-0">
                    {/* Prev Button */}
                    <button onClick={handlePrevStep} className="absolute left-[-48px] top-1/2 transform -translate-y-1/2 bg-green-200 text-green-800 rounded p-4 text-xl shadow-md z-10 hover:bg-green-300 transition-colors" disabled={currentStep === 0 && stepStarted}>
                        ←
                    </button>

                    <img src={story.image} alt={story.title} className="w-64 md:w-80 rounded shadow"/>
                    ...
                    <div className="max-w-xl text-lg text-gray-800 leading-relaxed">
                        ...
                        {stepCompleted && (<div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
                                <p className="text-green-700 font-medium">✅ Bu adımı tamamladın! Bir sonraki adıma geçebilirsin.</p>
                            </div>)}

                        {stepStarted && !isRecording && !stepCompleted && (<div className="mt-6 text-center">
                        ...
                        {stepStarted && !isRecording && !stepCompleted && (<div className="mt-6 flex justify-center">
                                <button onClick={handleRecord} className="relative w-32 h-32 bg-gradient-to-b from-red-400 to-red-600 rounded-full shadow-2xl hover:from-red-500 hover:to-red-700 transform hover:scale-105 transition-all duration-200 active:scale-95">
                        ...
                        {isRecording && (<div className="mt-6 flex justify-center">
                                <div className="relative w-40 h-40">
                                    {/* Background Circle */}
                                    <svg className="w-40 h-40 transform -rotate-90" viewBox="0 0 160 160">
                                        <circle cx="80" cy="80" r="70" fill="transparent" stroke="#e5e7eb" strokeWidth="8"/>
                                        {/* Progress Circle */}
                                        <circle cx="80" cy="80" r="70" fill="transparent" stroke="#ef4444" strokeWidth="8" strokeLinecap="round" strokeDasharray={2 * Math.PI * 70} strokeDashoffset={2 * Math.PI * 70 * (countdown / 10)} className="transition-all duration-1000 ease-linear"/>
                                    </svg>
                                    {/* Center Content */}
                    ...
                    {/* Next Button */}
                    <button onClick={handleNextStep} className="absolute right-[-48px] top-1/2 transform -translate-y-1/2 bg-green-200 text-green-800 rounded p-4 text-xl shadow-md z-10 hover:bg-green-300 transition-colors disabled:opacity-50" disabled={!stepCompleted && stepStarted}>
            ...
            <div className="flex items-center justify-center gap-6 px-6 py-6 bg-gray-50">
                {/* Next Step Button */}
                {stepCompleted && currentStep < steps.length - 1 && (<button onClick={handleNextStep} className="flex flex-col items-center bg-green-500 hover:bg-green-600 text-white rounded-2xl px-8 py-4 shadow-lg transform hover:scale-105 transition-all duration-200 active:scale-95">
                        <div className="text-4xl mb-2">➡️</div>
                        <div className="text-lg font-bold">SONRAKİ ADIM</div>
                    </button>)}

                {/* Complete Button */}
                {stepCompleted && currentStep === steps.length - 1 && (<button onClick={() => navigate('/')} className="flex flex-col items-center bg-purple-500 hover:bg-purple-600 text-white rounded-2xl px-8 py-4 shadow-lg transform hover:scale-105 transition-all duration-200 active:scale-95">
            ...
            <div className="fixed bottom-2 right-8 z-20 cursor-pointer transform hover:scale-105 transition-all duration-200" onClick={handleReplay}>
                <div className="relative">
                    ...
                    {mascotState === 'speaking' && (<div className="absolute top-4 right-4 animate-pulse">
                    ...
                    {mascotState === 'listening' && (<div className="absolute top-4 right-4">
                    ...
                    {mascotState === 'listening' && (<div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-yellow-400 text-black px-4 py-2 rounded-full text-sm font-bold animate-bounce shadow-lg">
                            📱 Tekrar dinlemek için tıkla!
                        </div>)}

                    {/* Idle state hint */}
                    {mascotState === 'idle' && stepStarted && (<div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-blue-400 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg">
                            👋 Ben DOST!
                        </div>)}
                </div>
            </div>
